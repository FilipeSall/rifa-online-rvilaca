              ...
              <div className="absolute right-4 top-[calc(100%-0.35rem)] w-[310px] overflow-hidden rounded-xl border border-neon-pink/30 bg-luxury-card/95 p-3 shadow-2xl backdrop-blur-md md:right-8 lg:right-8">
                ...
                <div className="space-y-2">
                  <button
                    ...
                    type="button"
                    onClick={handleSimpleOptionClick}
                  >
                    ...
                    <span className="flex-1">Acessar minha conta</span>
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ease-out ${
                    isSimpleFormOpen ? 'mt-3 max-h-[620px] opacity-100 translate-y-0' : 'mt-0 max-h-0 opacity-0 -translate-y-2 pointer-events-none'
                  }`}
                  aria-hidden={!isSimpleFormOpen}
                >
                  <form className="space-y-3" onSubmit={handleSimpleAuthSubmit}>
                    {!isCreatingAccount ? (
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          disabled={!isSimpleFormOpen || isSimpleSubmitting}
                          required={isSimpleFormOpen}
                        />
                      </div>
                    ) : null}
                    {isCreatingAccount ? (
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          disabled={!isSimpleFormOpen || isSimpleSubmitting}
                          required={isSimpleFormOpen && isCreatingAccount}
                        />
                      </div>
                    ) : null}
                    {isCreatingAccount ? (
                    ...
                    <button
                      ...
                      disabled={!isSimpleFormOpen || isSimpleSubmitting}
                    >
                      {isSimpleSubmitting ? (isCreatingAccount ? 'Criando conta...' : 'Entrando...') : (isCreatingAccount ? 'Criar conta' : 'Entrar')}
                    </button>
                    {simpleAuthError ? <p className="px-1 text-[11px] text-red-300">{simpleAuthError}</p> : null}
                  </form>
                </div>
              </div>
            ) : null}
